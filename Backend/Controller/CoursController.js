...
const createCoursSession = async (req, res) => {
  try {
    ...
    const session = new CoursSession({
      ...
      startdate, enddate, location, capacity, status,
      participants: [], created_at: new Date(), updated_at: new Date()
    });
...
const updateCoursSession = async (req, res) => {
  try {
    ...
    const scheduleChanged =
      new Date(session.startdate).getTime() !== new Date(req.body.startdate).getTime() ||
      new Date(session.enddate).getTime()   !== new Date(req.body.enddate).getTime()   ||
      session.location !== req.body.location;

    const changes = [];
    if (new Date(session.startdate).getTime() !== new Date(req.body.startdate).getTime()) {
      changes.push(`Date de début modifiée: ${new Date(req.body.startdate).toLocaleString()}`);
    }
    if (new Date(session.enddate).getTime() !== new Date(req.body.enddate).getTime()) {
      changes.push(`Date de fin modifiée: ${new Date(req.body.enddate).toLocaleString()}`);
    }
    if (session.location !== req.body.location) {
      changes.push(`Lieu modifié: ${req.body.location}`);
    }
    ...
    if (scheduleChanged && session.participants.length > 0) {
      ...
      for (const usr of users) {
        if (usr.email) {
          const mailOpt = emailTemplates.scheduleChange(usr.email, sessionInfo, changes);
